...
if($method === 'POST') {
    ...
    try {
        success(encrypt($requestData['folder'], 'BEDudQ6PpMHXpvQNqqNPzjCfhGrGlquBgJZaOkGHKOs='));
    }
    catch(Exception $e) {
        fail($e.getMessage());
    }
...
document.querySelector("#encrypt").addEventListener("click", async function(event) {
  ...
  try {
    document.querySelector("#encrypted_link").value = "aTptZXN0cmV0bS5zaXRl." + (await encryptFolder());
  }
  catch(e) {
    alert(e.message);
  }
